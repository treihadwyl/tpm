...
function install() {
    ...
    return new Promise( function( resolve, reject ) {
        ...
        install.stdout.on( 'data', logProgress );
        install.stderr.on( 'data', logProgress );

        install.on( 'error', function( err ) {
                logger.naked().logline();
...
function updateSource() {
    return new Promise( function( resolve, reject ) {
        file.get( 'banner.tmpl' )
            ...
            .then ( function() {
                return file.rimraf( 'banner.tmpl' );
            })
            .then( function() {
...
/**
 * Removes link to base-module on git
 */
function removeGit() {
    return new Promise( function( resolve, reject ) {
        file.rimraf( '.git/' )
            .then( function() {
                logger.log( 'cleaned git', chalk.green( '✔' ) );
                logger.log( chalk.yellow( 'Run' ), chalk.cyan( 'git init' ), chalk.yellow( 'to link to git' ) );
                resolve();
            })
            .catch( function( err ) {
                logger.log( 'Error unlinking from git', chalk.red( '✗' ) );
                if ( opts.debug ) {
                    logger.log( chalk.red( err ) );
                }
                reject( err );
