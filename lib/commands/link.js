...
function link() {
    ...
    config
        ...
        .then( function( res ) {
            ...
            return Promise.all([
                symlink( path.join( packagepath ), path.join( res.coreDir, linkpath, path.basename( packagepath ) ) ),
                symlink( path.join( packagepath ), path.join( res.coreDir, 'node_modules', path.basename( packagepath ) ) )
            ]);
        })
        .then( function() {
            logger.log( 'Links created', chalk.green( '✔' ) );
        })
        .catch( function( err ) {
